...
# Build cases data frame
cases <- NULL
# Loop over case-specific subsets
for (mrmcDF.cur in split(mrmcDF, mrmcDF$caseID)) {
  
  # Extract case ID and truth
  caseID <- mrmcDF.cur[1, "caseID"]
  truth <- mrmcDF.cur[1, "truth"]
  subStudyLabel <- mrmcDF.cur[1, "subStudyLabel"]
  score <- 0
  if (truth == "cancer") score <- 1

  # Create a row of the truth data frame
  cases.cur <- data.frame(
    readerID =  "truth",
    caseID = caseID,
    modalityID = "truth",
    score = score,
    truth = truth,
    subStudyLabel = subStudyLabel
  )
...
# Save data frame ##########
casesViper365 <- cases
write.csv(casesViper365, file = file.path("inst", "extdata", "casesViper365.csv"),
          row.names = FALSE)
usethis::use_data(casesViper365, overwrite = TRUE)
